...
if command -v greetd &> /dev/null; then
    ...
    # Ищем правильное имя команды cage (может быть cage или cage-git)
    CAGE_CMD="cage"
    if command -v cage-git &> /dev/null; then
        CAGE_CMD="cage-git"
    elif ! command -v cage &> /dev/null; then
        log_warning "cage/cage-git не найдены, используем fallback"
        CAGE_CMD=""
    fi
...
echo "📝 СЛЕДУЮЩИЕ ШАГИ:"
echo ""
echo "1️⃣  Смени обои и сгенерируй тему:"
echo "    ~/.config/waybar/theme.sh"
echo ""
echo "2️⃣  (Опционально) Отредактируй конфиги:"
echo "    nano ~/.config/niri/config.kdl"
echo "    nano ~/.config/waybar/config.jsonc"
echo ""
echo "3️⃣  Перезагрузи сессию (Mod+Shift+R в niri) или перезагрузись"
echo ""
